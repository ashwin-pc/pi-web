...
import type { DirectoryListing } from "../runtime/protocol.js";
...
export async function listDirectories(pathValue: unknown, fallback = process.cwd()): Promise<DirectoryListing & { ok: true }> {
...
export async function createDirectory(parent: string, name: string, fallback = process.cwd()) {
