...
export async function gitStatus(cwd = process.cwd(), fetchRemote = false) {
  if (!await isGitRepo(cwd)) return { ok: true as const, isRepo: false, ahead: 0, behind: 0, files: [] as Array<ReturnType<typeof parseStatusLine>> };
...
export async function readGitImage(options: { cwd: string; path: string; oldPath?: string; version: string; staged: boolean }) {
  ...
  if (!isImageGitPath(displayPath)) throw Object.assign(new Error("Not an image file"), { status: 415 });

  if (options.version === "before") {
    return { displayPath, data: await gitBuffer(["show", `HEAD:${oldPath || filePath}`], 15_000, options.cwd) };
  }
  if (options.version !== "after") throw new Error("Invalid image version");
  if (options.staged) {
    return { displayPath, data: await gitBuffer(["show", `:${filePath}`], 15_000, options.cwd) };
  }
  ...
  return { displayPath, data: await readFile(resolved) };
}

export async function gitImageBase64(options: { cwd: string; path: string; oldPath?: string; version: string; staged: boolean }) {
  const image = await readGitImage(options);
  return { ok: true as const, path: image.displayPath, base64: image.data.toString("base64") };
}

export async function gitCwdFromRepoParam(repo: string | null, baseCwd = process.cwd()) {
...
async function gitRepoSummary(path: string, cwd: string) {
  ...
  return {
    path,
    root: "root" in status ? status.root || cwd : cwd,
    branch: "branch" in status ? status.branch || "" : "",
    upstream: "upstream" in status ? status.upstream || "" : "",
...
export function parseCommit(entry: string) {
...
export async function gitLog(cwd = process.cwd()) {
  if (!await isGitRepo(cwd)) return { ok: true as const, isRepo: false, commits: [] as Array<ReturnType<typeof parseCommit>> };
  ...
  return { ok: true as const, isRepo: true, commits };
}
...
export async function gitDiff(options: { cwd?: string; path: string; staged?: boolean }) {
  ...
  if (!stdout) {
    ...
    if (file?.label === "untracked") {
      stdout = (await git(["diff", "--no-index", "--", "/dev/null", filePath], 15_000, cwd).catch((error: Error & { stdout?: string }) => ({ stdout: error.stdout || "" }))).stdout;
    }
  }
  return { ok: true as const, path: filePath, staged, diff: stdout };
}

export async function gitSync(cwd = process.cwd()) {
  if (!await isGitRepo(cwd)) throw Object.assign(new Error("Not a Git repository"), { status: 404 });
  const status = await gitStatus(cwd);
  const branch = "branch" in status ? status.branch : "";
  if (!branch) throw Object.assign(new Error("Cannot sync detached HEAD"), { status: 400 });
