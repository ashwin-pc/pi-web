...
export async function handlePasskeyRoute(req: IncomingMessage, res: ServerResponse, url: URL, kernel: AuthKernel, store: AuthStore, config: PasskeyConfig): Promise<boolean> {
  ...
  if (req.method === "POST" && url.pathname === "/api/auth/logout") { kernel.clearSession(res); json(res, 200, { ok: true }); return true; }
  if (req.method === "POST" && url.pathname === "/api/auth/passkey/options") {
    ...
    challenges.set(options.challenge, { challenge: options.challenge, expiresAt: Date.now() + 120_000, kind: "login" }); json(res, 200, options); return true;
  }
  if (req.method === "POST" && url.pathname === "/api/auth/passkey/verify") {
    const response = await body(req) as AuthenticationResponseJSON; const pending = challenges.get(response.response.clientDataJSON ? JSON.parse(Buffer.from(response.response.clientDataJSON, "base64url").toString()).challenge : "");
    ...
    await store.update(s => { const found = s.credentials.find(c => c.id === record.id); if (found) found.counter = result.authenticationInfo.newCounter; }); await kernel.establishSession(res, { id: "passkey:owner", displayName: "Owner" }); json(res, 200, { ok: true }); return true;
  }
  if (req.method === "POST" && url.pathname === "/api/auth/bootstrap/options") {
    if (!isLoopback(req)) { json(res, 403, { ok: false, error: "Bootstrap is localhost-only" }); return true; }
    ...
    challenges.set(options.challenge, { challenge: options.challenge, expiresAt: Date.now() + 120_000, kind: "register", bootstrapHash: state.bootstrap.hash }); json(res, 200, options); return true;
  }
  if (req.method === "POST" && url.pathname === "/api/auth/bootstrap/verify") {
    if (!isLoopback(req)) { json(res, 403, { ok: false, error: "Bootstrap is localhost-only" }); return true; }
    const input = await body(req) as { response: RegistrationResponseJSON; name?: string }; const client = JSON.parse(Buffer.from(input.response.response.clientDataJSON, "base64url").toString()); const pending = challenges.get(client.challenge);
    ...
    await store.update(s => { if (!s.bootstrap || s.bootstrap.hash !== pending.bootstrapHash || s.bootstrap.usedAt) throw new Error("Bootstrap already used"); s.bootstrap.usedAt = Date.now(); s.credentials.push({ id: c.id, publicKey: b64(c.publicKey), counter: c.counter, transports: c.transports, name: input.name?.slice(0, 80) || "Passkey", createdAt: Date.now() }); });
    await kernel.establishSession(res, { id: "passkey:owner", displayName: "Owner" }); json(res, 200, { ok: true }); return true;
  }
  return false;
}
