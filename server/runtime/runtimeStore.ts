...
function isCommandRuntimeConfig(value: unknown): value is CommandRunnerConfig {
  ...
  return typeof item.id === "string" && item.id.trim().length > 0
    ...
    && typeof item.modelBroker === "boolean"
    && (item.network === undefined || typeof item.network === "string")
    && (item.networkPolicy === undefined || item.networkPolicy === "none" || item.networkPolicy === "host-only" || item.networkPolicy === "provider-only" || item.networkPolicy === "unrestricted" || item.networkPolicy === "unverified" || item.networkPolicy === "unknown")
    && (item.networkVerifiedAt === undefined || typeof item.networkVerifiedAt === "string")
