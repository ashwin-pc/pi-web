...
export type AgentEventDto =
  ...
  | { type: "message_update"; assistantMessageEvent: JsonValue }
...
export function mapPiEvent(event: AgentSessionEvent): PiEventMapResult {
  switch (event.type) {
    ...
    case "agent_end": return { kind: "event", event: { type: "agent_end", willRetry: event.willRetry } };
    ...
    case "message_update": return { kind: "event", event: { type: "message_update", assistantMessageEvent: value(event.assistantMessageEvent) } };
    case "message_end": {
    ...
    case "auto_retry_start": return { kind: "event", event: { ...event } };
    case "auto_retry_end": return { kind: "event", event: { ...event } };
    case "summarization_retry_scheduled": return { kind: "event", event: { ...event } };
    case "summarization_retry_attempt_start": return { kind: "event", event: { ...event } };
    case "summarization_retry_finished": return { kind: "event", event: { type: "summarization_retry_finished" } };
