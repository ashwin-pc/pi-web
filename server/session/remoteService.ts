...
export class RemoteSessionService implements SessionService {
  ...
  /** Fire-and-forget serving adapter: true means accepted by transport, not handled remotely. */
  respondInteraction(response: Parameters<SessionService["respondInteraction"]>[0]) { void this.call("respondInteraction", [response]).catch(()=>{}); return !this.failure; }
  cancelInteractions() { void this.call("cancelInteractions", []).catch(()=>{}); }
