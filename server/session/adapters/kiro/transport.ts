...
export interface KiroTransportCallbacks {
...
export class KiroTransport {
  ...
  constructor(options: KiroLaunchOptions, private readonly callbacks: KiroTransportCallbacks) {
    this.maxFrameBytes = options.maxFrameBytes ?? 8 * 1024 * 1024;
    this.requestTimeoutMs = options.requestTimeoutMs ?? 45_000;
    this.child = spawn(options.command ?? "kiro-cli", options.args ?? ["acp", "--agent-engine", "v2"], {
      cwd: options.cwd,
    ...
    this.child.stdin.on("error", () => this.fail(new KiroRpcError("Kiro input closed", "closed", true)));
    this.child.once("error", () => this.fail(new KiroRpcError("Could not launch Kiro ACP; check the configured executable and native setup", "closed")));
    this.child.once("close", (code, signal) => {
      this.finish(new KiroRpcError(`Kiro ACP exited (${signal ?? code ?? "unknown"})`, "closed", true));
      void this.dispose();
    });
  }
  ...
  request(method: string, params: unknown = {}, timeoutMs: number | null = this.requestTimeoutMs): Promise<unknown> {
    if (this.ended) return Promise.reject(new KiroRpcError("Kiro ACP is closed", "closed"));
    ...
    return new Promise((resolve, reject) => {
      const timer = timeoutMs === null ? undefined : setTimeout(() => {
        ...
        reject(new KiroRpcError(`Kiro ${method} response timed out`, "timeout", true));
      }, timeoutMs);
      timer?.unref?.();
  ...
  respond(id: RpcId, result: unknown): void { this.send({ jsonrpc: "2.0", id, result }); }
  reject(id: RpcId, message = "This client does not support the required Kiro request", code = -32601): void {
    this.send({ jsonrpc: "2.0", id, error: { code, message: diagnostic(message) } });
  }

  private send(message: unknown): void {
    if (this.ended) throw new KiroRpcError("Kiro ACP is closed", "closed");
    const line = `${JSON.stringify(message)}\n`;
    if (Buffer.byteLength(line) > this.maxFrameBytes || this.child.stdin.writableLength > this.maxFrameBytes) {
      throw new KiroRpcError("Kiro outgoing frame or input backlog exceeds limit", "protocol");
    }
  ...
  private receive(text: string): void {
    ...
    while ((newline = this.buffer.indexOf("\n")) !== -1) {
      ...
      if (!message || message.jsonrpc !== "2.0") return this.invalidFrame(Buffer.byteLength(line));
      ...
      if (error) pending.reject(new KiroRpcError(diagnostic(error.message), typeof error.code === "number" ? error.code : -32603));
  ...
  private deliver(callback: () => void): boolean {
    try { callback(); }
    catch { this.fail(new KiroRpcError("Kiro protocol handler failed; its connection was closed", "protocol", true)); }
    return !this.ended;
  }

  private invalidFrame(bytes: number): void {
    if (!this.deliver(() => this.callbacks.observation("invalid-frame", bytes))) return;
    this.fail(new KiroRpcError("Invalid or oversized Kiro protocol message", "protocol", true));
  }

  private fail(error: KiroRpcError): void {
    this.finish(error);
    void this.dispose();
  }

  private finish(error: KiroRpcError): void {
    if (this.ended) return;
    this.ended = true;
    this.buffer = "";
  ...
  private async stop(): Promise<void> {
    this.finish(new KiroRpcError("Kiro connection disposed", "closed"));
    ...
    clearTimeout(kill);
    // A wrapper may exit while descendants still hold no stdio. Reap the owned
    // group too, never a process selected by name or an unrelated server port.
    this.signal("SIGTERM");
