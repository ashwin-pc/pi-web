...
export function createWebPanels(options: {
  ...
  async function invoke(event?: { action?: string; payload?: unknown; fields?: Record<string, string | string[]> }) {
    ...
    try {
      const res = await fetch("/api/web-panel/invoke", {
        method: "POST",
        headers: apiHeaders(),
        body: JSON.stringify({ sessionId: getSessionId(), key: entry.key, ...event }),
      });
  ...
  function actionTarget(event: Event) {
    return event.target instanceof Element
      ? event.target.closest<HTMLElement>("[data-web-panel-action]")
      : null;
  }
  ...
  body.addEventListener("click", (event) => {
    ...
    void invoke({
      action: target.dataset.webPanelAction || "",
      payload: parsePayload(target.dataset.webPanelPayload),
  ...
  body.addEventListener("submit", (event) => {
    ...
    void invoke({
      action: submitter?.dataset.webPanelAction || event.target.dataset.webPanelAction || "",
      payload: parsePayload(submitter?.dataset.webPanelPayload || event.target.dataset.webPanelPayload),
