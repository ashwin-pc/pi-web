...
function fakeSession(cwd: string, id: string, path = join(cwd, `${id}.jsonl`)): PiWebSession {
  ...
  session = {
    ...
    promptTemplates: [], resourceLoader: { getSkills: () => ({ skills: [] }) }, bindExtensions: async () => {}, getActiveToolNames: () => ["read"],
    ...
    setSessionName: (name: string) => { session.sessionName = name || undefined; }, setModel: async () => {}, setThinkingLevel: () => {}, abort: async () => {},
    navigateTree: async () => ({ cancelled: false, editorText: "" }), subscribe: (fn: any) => { listeners.add(fn); return () => listeners.delete(fn); },
    prompt: async (message: string) => { const value = { role: "user", content: message, timestamp: "2026-01-01T00:00:00Z" }; entries.push({ id: `entry-${entries.length + 1}`, parentId: entries.at(-1)?.id || null, type: "message", timestamp: value.timestamp, message: value }); sync(); listeners.forEach(fn => fn({ type: "message_end", message: value })); },
    dispose: () => {},
  };
...
export async function createDeterministicLocalSessionService(cwd: string) {
  ...
  const deps: LocalSessionServiceDependencies = { modelRuntime: {} as any, sessionFactory: factory, additionalExtensionPaths: () => [], sessionConfig: { defaultsFor: async () => ({}), finalizeCreatedSession: async () => {} }, globalCwd: () => cwd, clientCount: () => 0 };
