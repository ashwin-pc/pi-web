...
export interface CodexPeer { pid: number; directory: string }
...
export type PeerControl = {
  action: "configure" | "accept" | "reject" | "release" | "emit" | "raw" | "text" | "thinking" | "tool" | "approval" | "complete" | "activity" | "error" | "exit";
  [key: string]: unknown;
};

const sequences = new Map<string, number>();

export async function readObserved(peer: CodexPeer): Promise<PeerObservation[]> {
...
export async function waitObserved(
  ...
  do {
    ...
    if (error) throw new Error(`Codex peer fixture error: ${JSON.stringify(error.message)}`);
...
export async function findPeer(
  ...
  do {
    for (const name of await readdir(join(root, "peers")).catch(() => [])) {
      ...
      try {
        ...
        const peer = JSON.parse(await readFile(join(directory, "ready.json"), "utf8")) as CodexPeer;
  ...
  throw new Error("Timed out locating the Codex native peer");
}

export function peerForThread(root: string, threadId: string, timeoutMs?: number): Promise<CodexPeer> {
  return findPeer(root, (record) =>
    (record.direction === "server" && record.message.method === "thread/started" && record.message.params?.thread?.id === threadId)
    || (record.direction === "client" && record.message.method === "thread/resume" && record.message.params?.threadId === threadId), timeoutMs);
}

/** Atomic numbered command files avoid partial reads and do not require a control server. */
export async function controlPeer(peer: CodexPeer, command: PeerControl): Promise<void> {
...
export async function acceptedTurn(peer: CodexPeer, afterTurnId?: string): Promise<{ threadId: string; turnId: string }> {
  const record = await waitObserved(peer, (value) => value.direction === "server" && value.message.method === "turn/started" && value.message.params.turn.id !== afterTurnId);
  return { threadId: record.message.params.threadId as string, turnId: record.message.params.turn.id as string };
}
