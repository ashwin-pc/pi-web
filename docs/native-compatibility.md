...
**Last independent verdict: FAIL on code `47e14fb075a57197d2ac21654b4c1e45e153b4fb`, docs `1a205c8`, 2026-09-14.** Full regressions and original counterexamples passed/adjudicated, but Claude's explicit-executable `--version` preflight still inherited `PI_WEB_TOKEN`; the runtime-only exclusion claim did not cover it. **Repair `2c5fe71` now filters both launch paths and adds a production-adapter regression; fresh independent acceptance is required.** This is review-branch code, not published npm `0.6.0`, upstream `main` or deployment approval. See [the implemented design](multi-harness-design.md), [Codex native details](codex-native.md), [Claude native details](claude-native.md), and [#92](https://github.com/ashwin-pc/pi-web/issues/92).
...
| Native children receive host control credential | **Runtime fix `8788bb0` was incomplete; B1 preflight repair `2c5fe71` awaits fresh independent acceptance.** At `47e14fb`, both runtime children were filtered, but Claude's earlier explicit-executable `--version` child was not. The existing helper now covers that child too, preserving native auth/config/Pi and immutable inputs. The new matching-version adapter regression and both unchanged audit probes are producer-green. This is launcher evidence, not a model/tool/API exploit or an OS sandbox claim. |
...
| Final assembled validation | **Independent full-suite evidence verified at `47e14fb`, but overall verdict FAIL due to B1.** Typecheck/build, 797 unit passes / 2 skips and 888 browser passes / 55 skips had zero failures/retries. The auditor's fresh 272-unit / 42-browser spot-checks also passed, while new version-child probes failed. A new verdict must evaluate the repaired candidate; no main promotion or deployment is claimed. |
...
| B1 repair `2c5fe71`, producer | Two new matching-version adapter cases red → green, asserting exclusion in both children. Both unchanged audit probes (accepted and rejected version, inherited and explicit env) green twice. Project/strict typechecks and **73 focused passes / 2 opt-in skips**. Complete build and full parallel `npm test` on the repair: **799 unit passes / 2 skips; 888 browser passes / 55 skips; zero failures/retries**. | Fresh independent acceptance, actual model calls or native tool/API exploitation. The prior runtime-only probe remains valid only for its narrower boundary. |
...
The independent verdict on `47e14fb` is **FAIL**, despite green full regressions and adjudicated original counterexamples. Repair `2c5fe71` addresses its blocking preflight omission. The repair's producer full suite and new accepted-version **both-children** regression pass; a fresh explicit independent verdict is still required. Original runtime-env checks must not again stand in for the earlier version launch. Documentation/comment corrections do not certify another runtime rerun.
