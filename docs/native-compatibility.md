...
**Frozen code target reviewed:** integration `47e14fb075a57197d2ac21654b4c1e45e153b4fb`, 2026-09-14. It includes the repaired consent/null handling, native host-token exclusion, Pi abort presentation and native drawer-test readiness, plus the isolated Chromium-cache recipe. This is review-branch code, not published npm `0.6.0` or upstream `main`. The final independent suite/counterexample rerun and acceptance verdict are **pending**; producer passes below do not replace them. See [the implemented design](multi-harness-design.md), [Codex native details](codex-native.md), [Claude native details](claude-native.md), and [#92](https://github.com/ashwin-pc/pi-web/issues/92).

## Acceptance status first

| Finding / gate | Status at this source pin |
| --- | --- |
| Stopped Pi prose disappears | **Fixed in `b312d6d` (owned `5c2db66`); final independent recheck pending.** Explicit abort now preserves retained parts instead of preferring a transport diagnostic/failure card. Attribution is an inherited baseline defect: the affected projection was byte-identical to `f7967b8`. Captured-data projection, public-SDK cold-open replay and browser live/reload regressions are producer-green. The original actual canary still failed its UI assertion and **did not reach cold service restart**. |
| Native children receive host control credential | **Fixed in `8788bb0` (owned `814055a`); final independent recheck pending.** Final Codex/Claude spawn environments exclude only case-insensitive `PI_WEB_TOKEN`, without mutating inputs or changing native auth/config/Pi behavior. The unchanged independent probe was producer-red → green twice; controlled OS peers cover default/explicit/custom launches and Codex create/list/resume. This is not a model/tool/API exploit or an OS sandbox claim. |
| Codex consent context | **Fixed in `617ab3f` (owned `4466927`).** The original two context probes independently passed twice, with 24 native-peer browser passes at `617ab3f`. Final frozen-target recheck/acceptance remains pending. Conservative context/URL/scope limits remain; actual prompted approval was not encountered. |
| Codex optional/null permission fields | **Fixed in `2c5101c` (owned `ec3eade`); final independent recheck pending.** The previous validator rejected four schema-valid optional/null cases. The unchanged six-case schema/process probe is producer-green twice. Nested representations and native scope/response mappings are preserved; no defaults or broader grants are introduced. |
| Native drawer helper readiness/focus | **Fixed in `47e14fb` (owned `5aafef8`, test-only); final independent recheck pending.** The causal delay regression now awaits drawer hydration/overlay closure, then uses normal pointer focus. Desktop keeps its drawer; landing does not await a drawer-only GET. The old hidden-Send failure remains in the record, not waived. Producer's final repeated focused matrix passed 72/72 with zero retries. |
| Claude lifecycle corrections | **Fixed in `4a33204` / `0fbe2c1` (owned `e691238` / `ef6b994`).** Unchanged independent recovery/stale-interrupt/unfinished-tool probes were **4/4 green twice on `0fbe2c1`**. Idle opt-in/abort classification also have real-CLI/no-real-model regressions. Final frozen-target recheck is pending; this is not a fresh paid full-flow rerun. |
| Final assembled validation | **Producer pass; independent final gate pending on `47e14fb`.** Final UI producer reports typecheck/build, 797 unit passes / 2 skips and 888 browser passes / 55 skips, zero failures/retries. Earlier counts retain their exact pins below. No independent final correctness/simplicity approval, main promotion or deployment is claimed. |
...
| Host control credential in native child environment | Host environment unchanged; SDK/extension behavior retained | Excluded at final `CodexTransport` spawn | Excluded at final SDK callback, including supported custom spawn | `nativeChildEnvironment` → copied spawn env, no browser consumer. **native-env** verifies exact-key/case-alias exclusion, preserved native sentinels and immutable parent/caller/SDK inputs; actual Pi SDK local command still sees its host variable. `8788bb0` is narrow credential separation, not host-filesystem isolation. |
...
review_commit=47e14fb075a57197d2ac21654b4c1e45e153b4fb
...
Use the terminal's single-use setup link and normal browser password/passkey sign-in; [authentication instructions](passkey-auth.md) cover recovery. Do not use the fixtures' open/no-auth policy for a real-harness acceptance check. Treat bootstrap URLs/server logs as private, not publishable canary evidence. Clearing inherited web auth here avoids sharing the live service's credentials. Separately, `8788bb0` filters the host token from the final native child environment even when the server has one; this does not establish host-filesystem or general secret isolation.
...
The assigned repairs and their regressions are integrated in frozen code target `47e14fb`; final independent acceptance is not yet established. The remaining code gate is a fresh isolated ci/typecheck/build/full `npm test` plus all unchanged original probes (Claude lifecycle, Codex consent and six-case optional/null handling, native token, drawer focus, Pi captured projection), followed by an explicit independent correctness **and** simplicity verdict. This refresh changes documentation only and does not certify a code rerun.
