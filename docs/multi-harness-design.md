...
**Frozen code target:** `47e14fb075a57197d2ac21654b4c1e45e153b4fb`, 2026-09-14. It includes the Pi abort-display repair (`b312d6d`), native drawer-test readiness repair (`47e14fb`), native host-token exclusion (`8788bb0`), Codex optional/null permission repair (`2c5101c`) and isolated-browser-cache recipe (`571b384`). These previously recorded findings are **fixed in the target, with final independent rechecks/acceptance pending**. The [compatibility matrix](native-compatibility.md) separates implementation, producer results, earlier independent checks and still-unrun actual/platform gates.
...
| [`adapters/nativeEnvironment.ts`](../server/session/adapters/nativeEnvironment.ts) | Copies the final Codex/Claude child environment and excludes the case-insensitive exact `PI_WEB_TOKEN` key, after SDK assembly/updates. Preserves other native auth/configuration and input maps; Pi does not use this helper. Not an OS sandbox or a general environment policy. |
...
Browser authentication and native provider authentication are separate. No new native login UI is added. Unknown ingress diagnostics omit/redact payloads, and arbitrary auth output is not a transcript feature. **The host-token inheritance defect is repaired in `8788bb0`** (owned `814055a`): the final Codex spawn and Claude SDK callback receive copied environments without `PI_WEB_TOKEN`, including case variants. Native credentials/profile/HOME, explicit-environment replacement semantics and Pi's host environment remain unchanged. The original presence-only probe reproduced red, then passed twice in the producer tree; process-peer tests also cover create/list/resume, explicit/custom launches and immutable inputs. Final independent recheck is pending. This is narrow credential separation, not isolation of the host filesystem, every secret or arbitrary trusted custom spawn code.
...
Use the [setup and validation recipes](native-compatibility.md#setup-and-reproducible-validation) with an explicit Chromium cache, non-artifact validation cwd and free owned ports outside the platform's ephemeral range. The final UI producer reports typecheck/build, 72 repeated focused browser passes and full `npm test` (797 units / 2 skips; 888 browser passes / 55 skips; zero retries). That is producer evidence for the integrated code, **not the pending independent full-suite/counterexample rerun or correctness/simplicity verdict on `47e14fb`**. Actual Pi cold restart, prompted native approvals and canonical macOS/Windows execution remain unrun; no paid budget or deployment authorization is implied.
