...
## Upgrade: proxies that rewrite Host

Before upgrading, preserve the public `Host` header or set `PI_WEB_AUTH_ORIGIN` to the exact public origin. Alternatively, explicitly configure `PI_WEB_AUTH_PROXY_PEERS` with exact proxy IPs: only those socket peers may supply one strictly parsed `X-Forwarded-Host` authority (no chains). The same canonical IP trust applies to X-Forwarded-For, including IPv4-mapped IPv6 sockets. Proxies must overwrite caller-supplied forwarding headers; restrict direct backend access. Untrusted forwarding headers never authorize an origin. Rejections carry an actionable hint and emit at most one fixed diagnostic per process. HTTP, public auth, WebSockets and supervisor share this rule; the supervisor preserves the validated authority without laundering untrusted forwarding headers through its loopback connection.
...
4. Choose **Retire** beside Legacy token, or disable `legacy` under **Sign-in options & legacy retirement**. Confirm the retirement. It is rejected unless a remaining enabled, enrolled method has a verified login.
...
Password/legacy/external login and password-bootstrap attempts share a bounded per-store, per-peer failure cooldown (100ms escalating to at most two seconds), with at most one concurrent login handler. All scrypt work, including password rotation, has a single process-wide slot; existing N=131072 hashes remain supported, bounded to one verification at a time. A busy slot returns a retryable failure rather than queuing unbounded work. The peer defaults to the actual socket address. Operators may explicitly set `PI_WEB_AUTH_PROXY_PEERS=127.0.0.1,::1` to trust a proxy that overwrites X-Forwarded-For with exactly one validated client IP. Loopback alone does not imply proxy trust; never enable this unless direct backend access is restricted and the proxy strips supplied headers. Setup validation errors do not consume the credential-failure cooldown. Rate limits are in-memory and reset on process restart. Body size and password length are bounded. Internet-facing deployments should also apply edge rate limiting.

API-token minting, device-grant minting, revoking another device, password changes, method changes, revoke-all, passkey enrollment and deletion require a credential-backed sign-in within five minutes. The Security page has an explicit sign-in link and, only while legacy remains enabled, a user-click saved-token reauthentication button (never an automatic retry); sign in, reopen Security, and retry. Grant sessions and historical sessions lacking reauthentication metadata cannot satisfy this gate. This is recent authentication, not MFA. An authenticated browser may set/change the owner password. Rotation revokes every *other* browser session and preserves the initiating session; Settings states this policy explicitly. Use **Sign out all devices** when all devices, including the current one, must sign out. Rotation and revoke-all cancel pending device grants and default to revoking all API tokens atomically with the sessions; active token WebSockets are closed by the same store listener. The password view and sign-out confirmation offer a checked **Also revoke API tokens** option. Unchecking it explicitly preserves automation access, including any stolen token: do not uncheck during suspected compromise. API clients may send `revokeApiTokens: false`; omission defaults to secure revocation. Self-revocation/logout needs no recent sign-in. Password setup/change enables password login but a fresh successful password login is required before it qualifies as a verified replacement.
...
**Settings → Security** opens a responsive overview, with focused views for passkeys, password changes, device links, API tokens, and advanced sign-in options. Loading and request failures appear inside Security, including on mobile. Passkeys run directly from the public sign-in page; password entry expands in place, and only enabled, ready alternatives are offered. Successful self/all-device revocation and passkey deletion clear the saved browser token and navigate to explicit sign-in; cancellation or failed requests do not claim sign-out.
