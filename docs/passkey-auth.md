...
```sh
pi-web auth bootstrap                 # single-use, 10 minute localhost URL
pi-web auth bootstrap --minutes 5
pi-web auth list
```

`recover` creates the same localhost-only, single-use enrollment primitive. After recovery, revoke the lost credential (which also revokes every browser session):
...
API tokens cannot enroll passkeys or become browser sessions.
