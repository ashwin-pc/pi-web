...
Limits are additive and fail closed: `maxSeconds` defaults to and is clamped to 120; `maxBytes` defaults to and is clamped to 25 MB. MIME types must be `audio/*`. Core bounds bytes and the host-reported recording duration; extensions that decode untrusted media must also validate the actual container, codec, and decoded duration before transcription. Each upload is bound to its session, contribution key, and opaque registration revision (replacing a contribution invalidates captures from the previous registration), can be consumed by one invocation only, lives in a mode-0700 temporary directory as a mode-0600 file, and is deleted when the invocation finishes (with a five-minute expiry as fallback). The browser-provided capture path is ignored; the server injects the validated `{ path, mimeType, size, durationMs }` record.
...
The [local dictation example](../examples/pi-web-extensions/dictation/README.md) provides private, on-device transcription on Apple Silicon with selectable, isolated Parakeet MLX and Whisper MLX adapters. Runtime packages and model IDs are configurable; model weights remain outside the repository in a user-managed path or external Hugging Face cache.
