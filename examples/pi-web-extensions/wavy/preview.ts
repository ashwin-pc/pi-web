...
export async function readWavyPreviewAssets(root = here, includeNotation = true) {
  try {
    // Read the generated bundle first so a source checkout reports the canonical
    // build instruction rather than racing an unrelated optional resource error.
    const browser = await readFile(join(root, "browser.js"), "utf8");
    const [css, abcjs] = await Promise.all([
...
/** Preserve trusted script semantics through the host HTML normalizer, which strips literal C0 bytes. */
export function normalizationStableScript(source: string): string {
  return source.replace(/[\u0000-\u0008\u000B\u000C\u000E-\u001F\u007F]/g, character => `\\x${character.charCodeAt(0).toString(16).padStart(2, "0")}`);
}
...
export async function renderWavyView(project: LoadedProject): Promise<string> {
  const scoreRef = project.index.revisions.at(-1)?.files.score;
  const hasRenderableScore = isRenderableScore(project.head.score);
  const [css, browser, abcjsSource] = await readWavyPreviewAssets(here, hasRenderableScore);
  const abcjs = normalizationStableScript(abcjsSource);
  ...
  const takes = project.index.takes.map(t => {
    const audio = t.audio ?? t.wav;
    return { ...t, audio: audio ? file(audio) : undefined };
  });
...
export async function renderWavyPreview(project: LoadedProject): Promise<string> { return renderWavyView(project); }
